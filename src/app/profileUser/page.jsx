...
const getUserRestaurants = async (userId, owner) => {
  if (owner) {
    ...
    const res = await axios(`${process.env.NEXTAUTH_URL}/api/reviews/usuario/${userId}`);
    return res.data;
  }
};
...
export default async function ProfileUser() {
 ...
 await axios.get(`${process.env.NEXTAUTH_URL}/api/users/${id}`)
  .then( res => {
    ...
    <div className=' w-full  '>
      ...
      <div className='w-full max-w-[1400px] flex max-md:items-center max-md:flex-col m-auto gap-16 min-h-screen relative p-3'>
        ...
        <div className='w-[80%] mt-8'>
          ...
          >
            <h3>{Profiledate.name} {Profiledate.lastName}</h3>|<h3>Comensal frecuente</h3><ActualizarPerfil PerfilDatos={Profiledate}/>
          </div>
